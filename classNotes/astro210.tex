...
\subsection{Emergence of modern Astro}
...
\subsection{CMB}
\begin{list}{-}{}
\item \(ct \rightrightarrows\) theoretical size of the universe
\item Does not account for expansion of the uni while the light is travelling
\item \gls{surf-of-last-scatterting}, point where the universe becomes opaque, also the CMB
\end{list}

\subsubsection{olbers paradox}
\begin{list}{-}{}
\item We do not see a galaxy everywhere we look 
\item this is because the universe is finite, not infinitly large 
\end{list}

\subsubsection{Equivalence principle}


\begin{eqboxed}{grav-mass}{Gravitational Mass}
{}
F_{grav} = \frac{-GM_gm_g}{r^2}
\end{eqboxed}
\begin{eqboxed}{inert-mass}{Interial mass}
{}
a = \frac{F}{m_i}
\end{eqboxed}

\begin{eqboxed}{equiv-princp}{Equivalence Principle}
{}
m_g = m_i
\end{eqboxed}

\subsubsection{Einstein v Newton}
\begin{list}{-}{}
\item In a small volume of space the downward pull of gravity cannot be distinguished from an upward acceleration of the observer
\end{list}

\subsection{General relativity}
\begin{list}{-}{}
\item Space and time bind together to form curved spacetime 
\item `free-fall' is motion in a straight line in 4d spacetime (this is grav acceleration)
\item Matter (Mass-energy) tells spacetime how to curve 
\item Curved spacetime tells matter how to move
\end{list}

\subsubsection{Predictions of GR}
\begin{list}{-}{}
\item Gravtional field sdeflect light (grav lensing)
\item more accurate orbit for Mercury
\item grav fields slow clocks 
\item moving objects will produce gravitational waves
\end{list}
\begin{center} 
  \huge
  \begin{tikzpicture}
...
\subsection{Surface of spacetime}
\begin{list}{-}{}
\item How do we know the curvature and angles?
\item If we draw a triangle what do the angles add up to?

\end{list}
\begin{eqboxed}{curv-and-angles}{Curvature and Angles of a triangle in 4d space}
{Where \(\alpha + \beta + \gamma\) are angles of an equilateral triangle. \\Flat is \(\kappa = 0\), spherical is \(\kappa = 1\), Hyperbolic is \(\kappa = -1\)  }
\alpha + \beta + \gamma = \pi + \frac{\kappa A}{r^2_{c,0}}
\end{eqboxed}

\begin{eqboxed}{newt-friedmann-equation}{Newtonian Friedmann Equation}
{Newtonian version of Friedman equation}
\frac{\dot{a}}{a}^2 = \frac{8\pi G }{3} \rho(t + 2\frac{k}{r^2} \frac{1}{a^2(t)})
\end{eqboxed}

\begin{eqboxed}{rel-friedmann-equation}{Relativistic Friedmann equation}
...
\subsubsection{Measuring curvature}
\begin{list}{-}{}
\item Look at angular diam vs distance 
\item We look at something which we know the size of, a `standard ruler'
\item if flat\dots \(\Rightarrow \alpha = \frac{D}{d}\)
\item Can't use galaxies, non standardized size  
\item Use sound saves before recombination 
\begin{list}{-}{}
\item Use sound waves! \(t< 380000 yr \)
\item Sound waves in early fluid like plasma in universe 
\item wave equation for sound waves relates wavelength to the properties of the gas
\end{list}
...
\end{list}
\subsubsection{Predicting angular scale distribution in CMB from sound waves}
\begin{list}{-}{}
\item large fluctuations are caused by patches of varying temperature
\item small fluctuations are from
\item Look at how CMB is `distorted' from the sound waves
\item Then infer wavelength of soundwaves
\item Then apply previous logic for measuring angular res and comparing it to known wavelength  
\end{list}

\subsection{Density of the universe}
\begin{list}{-}{}
\item \[\Omega_m = \frac{M/V}{\rho_c}\]
\item Matter \dots
\item \[\Omega_m \sim .3\]
\item Radiation \dots
\item \[\Omega_{rad} \sim .00007\]
\item Something else \dots 
\item \[\Omega_{rest} \sim .7\]
\item Because the universe is flat we know that 
\item \[\Omega_{m} + \Omega_{rad} + \Omega_{res} \sim 1 \]
\end{list}

\subsubsection{Energy density components}
\begin{list}{-}{}
\item Looking for a solution for \(a(t)\)
\item Wavelength of a photon scales with the size of the universe
\item Lambda cosmological constant 
\item \[\Omega_\Lambda \sim .7\]
\item Non-Relativistic particles: 
\item \[\Omega_{m,0} \sim .3\]
\item \[\Omega_{(bary,0)} \sim .04\]
\item Relativistic particles 
\item \[\Omega_{r,0} = \Omega_{cmb,0} + \Omega_{v,0} \sim 8.4 \times 10^{-5}\]
\end{list}

\begin{eqboxed}{nonrelparticles}{Behavior of nonrelatvistic partciles}
{If matter is conserved}
n(t) \propto V^{-1}(t)\propto a^{-3}(t)
\end{eqboxed}

\subsubsection{Cosmological redshift}
\begin{eqboxed}{redshift}{Redshift Z}
{}
Z = \frac{\lambda_0 - \lambda_e}{\lambda_e}
\end{eqboxed}

If we use \ref{redshift} combined with expansion of the universe \dots  
\[1+Z = \frac{1}{a(t)}\]

\subsubsection{Universe at different stages}
\begin{list}{-}{}
\item Solve \ref{rel-friedmann-equation} for different \(\Omega\)s
\item We find that all are decelerating, except for \(\Omega_\Lambda\) (Dark Energy)
\end{list}

\begin{eqboxed}{eq:friedmannConsensusModel}{Friedmann Consensus Model}
{unitless acceleration of the universe}
\dot{a} = H_0\left[\frac{\Omega_{r,0}}{a^2} + \frac{\Omega_{m,0}}{a}+ \Omega_{\lambda,0}a^2\right]^{1/2}
\end{eqboxed}

\subsubsection{Standard candles for expanding universe}

\begin{eqboxed}{eq:flux}{Flux}
{}
L = \frac{L}{4\pi r^2}
\end{eqboxed}

\begin{eqboxed}{eq:photonenergy}{Photon Energy}
{}
...
\begin{list}{-}{}
\item One can combine \ref{redshift} and \refeq{eq:photonenergy}
\item We can use type 1a SN as a standard candle
\item Plot the redshift and distance, see what expected path it lies on (based on DE DM content)
\end{list}
